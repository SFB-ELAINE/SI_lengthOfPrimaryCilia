...
plotAutomaticDetection_Cultivation <- function(input_file,
  ...
  plot_total_length_violin <- ggplot(df_results_automatic_filtered, aes(x=cultivation, y=vertical_length_in_um)) +
    geom_violin() +
    # stat_boxplot(geom ='errorbar', width = 0.3) +
    geom_boxplot(width=0.1) +
    stat_summary(fun=mean, geom="point", size = 3, shape=23, color="black", fill="black") +
  ...
  for(i in length(names_of_experiments)){
    ...
    if(test_result$p.value <= 0.05){
      print(paste("We have a p-value <= 0.05 for the results of total lengths of ", names_of_experiments[i],".", sep=""))
      all_normal <- FALSE
    }
  }
  ...
  if(all_normal){
    print("All measurements (total lengths) are normally distributed according to the Shapiro-Wilk test.")
  }
  ...
  for(i in length(names_of_experiments)){
    ...
    test_result <- shapiro.test(lengths_for_statistical_tests)
    if(test_result$p.value <= 0.05){
      print(paste("We have a p-value <= 0.05 for the results of horizontal length of ", names_of_experiments[i],".", sep=""))
      all_normal <- FALSE
    }
  }
  ...
  if(all_normal){
    print("All measurements (horizontal lengths) are normally distributed according to the Shapiro-Wilk test.")
  }
  ...
  res.aov <- df_results_automatic_filtered %>% anova_test(horizontal_length_in_um ~ cultivation)
  ...
  if(res.aov$`p<.05` == "*"){
    ...
    plot_total_length_violin_t_test <- ggviolin(df_results_automatic_filtered, x = "cultivation", y = "horizontal_length_in_um",
                                                add = "boxplot", add.params = list(fill = "white")) +
      stat_summary(fun=mean, geom="point", size = 3, shape=23, color="black", fill="black") +
      stat_pvalue_manual(pairwise_comparison, label = "p.adj", tip.length = 0.01, step.increase = 0.05) +
      ylim(0,8) +
    ...
    ggsave(filename = paste(output_dir, "all_filtered_cilia_horizontal_lengths_violin_plot_t_test.pdf", sep="/"),
           width = 297, height = 210, units = "mm")
    ggsave(filename = paste(output_dir, "all_filtered_cilia_horizontal_lengths_violin_plot_t_test.png", sep="/"),
           width = 297, height = 210, units = "mm")
  ...
  for(i in length(names_of_experiments)){
    ...
    test_result <- shapiro.test(lengths_for_statistical_tests)
    if(test_result$p.value <= 0.05){
      print(paste("We have a p-value <= 0.05 for the results of vertical length of ", names_of_experiments[i],".", sep=""))
      all_normal <- FALSE
    }
  }
  ...
  if(all_normal){
    print("All measurements (vertical lengths) are normally distributed according to the Shapiro-Wilk test.")
  }
  ...
  res.aov <- df_results_automatic_filtered %>% anova_test(vertical_length_in_um ~ cultivation)
  ...
  if(res.aov$`p<.05` == "*"){
    ...
    plot_total_length_violin_t_test <- ggviolin(df_results_automatic_filtered, x = "cultivation", y = "vertical_length_in_um",
                                                add = "boxplot", add.params = list(fill = "white")) +
      stat_summary(fun=mean, geom="point", size = 3, shape=23, color="black", fill="black") +
      stat_pvalue_manual(pairwise_comparison, label = "p.adj", tip.length = 0.01, step.increase = 0.05) +
      labs(
    ...
    ggsave(filename = paste(output_dir, "all_filtered_cilia_vertical_lengths_violin_plot_t_test.pdf", sep="/"),
           width = 297, height = 210, units = "mm")
    ggsave(filename = paste(output_dir, "all_filtered_cilia_vertical_lengths_violin_plot_t_test.png", sep="/"),
           width = 297, height = 210, units = "mm")
